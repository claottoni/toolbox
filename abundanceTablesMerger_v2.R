...
for (i in 1:length(tabn)){
  ...
  if (!exists("tabfinal1")){
    tabfinal1 = merge(tab1, data, by=c(1,2), all=T)
    print(paste0("parsing ", files[c(1,2)]))
    } else {
  # merge all the others
  tabfinal1 = merge(tabfinal1, data, by=c(1,2), all=T)
  print(paste0("Merging ", tabn[i]))}
}
# remove NA
tabfinal1[is.na(tabfinal1)] <- 0
write.table(tabfinal1, file = 'abundance_table_IDs.merged', sep="\t", row.names=F, col.names=T, quote = FALSE)
