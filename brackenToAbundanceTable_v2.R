...
tab1.bracken = tab1[,c(1,2,6)]
# assign column names from filenames
names(tab1.bracken) = c("taxon","ID",basename(files[1]))
...
for (i in 1:length(tabn)){
  ...
  data.bracken = data[,c(1,2,6)]
  names(data.bracken) = c("taxon","ID",basename(tabn[i]))
  # at first, merge the first two files, then merge all the rest
  if (!exists("tabfinal1")){
    tabfinal1 = merge(tab1.bracken, data.bracken, by=c("taxon","ID"), all=T)
    print(paste0("parsing ", files[c(1,2)]))
    } else {
  # merge all the others
  tabfinal1 = merge(tabfinal1, data.bracken, by=c("taxon","ID"), all=T)
  print(paste0("parsing ", tabn[i]))}
}
# remove NA
tabfinal1[is.na(tabfinal1)] <- 0
write.table(tabfinal1, file = paste(dir,'taxa_abundance_bracken_names_IDs.txt',sep='/'),sep="\t",row.names=F, quote = FALSE)
